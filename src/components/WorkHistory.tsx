  ...
  return (
    <div style={{ background: "#FFFFFF", borderRadius: "16px", border: "1px solid #E8E8E8", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", padding: "32px" }}>
      ...
      {showStep4 && (
        <>
          <SectionConnector />
          <StepHeader n={4} title="Gap Resolution" hint="One or more gaps of 6+ months detected in the timeline" />
            ...
            {coverage.gaps.filter((g) => g.months >= 6).map((g, i) => (
              <div key={i} style={{ background: "#FFF5F5", borderLeft: "4px solid #C8202A", borderRadius: "0 10px 10px 0", padding: "12px 16px" }}>
                ...
                <ul style={{ margin: "8px 0 0 0", paddingLeft: "20px", color: "#4B4B4B", fontSize: "0.8125rem", lineHeight: 1.7 }}>
                  <li><strong>Add the missing period above</strong> — W2, SE, or gig employment that covers this window.</li>
                  <li><strong>Employer verification letter</strong> — cash employer letter covers the gap (amber — obtain before closing).</li>
                  <li><strong>Amend taxes</strong> — if income was earned but not reported. Consult CPA.</li>
                  <li><strong>Wait</strong> — 6 months continuous employment required. Earliest eligible: {ymLabel(addMonths(todayYM(), 6))}.</li>
                </ul>
              </div>
            ))}
          </div>
